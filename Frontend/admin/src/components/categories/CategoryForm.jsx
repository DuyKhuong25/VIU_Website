...
function CategoryForm({ category, onSave, onCancel, allCategories, isOpen }) {
    console.log("Rendering all category:", allCategories);
    ...
    useEffect(() => {
        if (isOpen) {
            if (category) {
                const vi = category.translations.find(t => t.languageCode === 'vi') || {};
                ...
                setFormData({ parentId: null, showOnHomepage: false, viName: '', enName: '' });
            }
            setErrors({});
        }
    }, [category, isOpen]);
    ...
    const handleChange = (e) => {
        ...
        if (name === 'parentId' && value) {
            newFormData.showOnHomepage = false;
        }
    ...
    const handleTranslate = async () => {
        ...
        try {
            ...
            if (errors.enName) {
                setErrors(prev => ({ ...prev, enName: null }));
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        const dataToSend = {
            ...
            showOnHomepage: formData.parentId ? false : formData.showOnHomepage,
            translations: [
    ...
    const renderCategoryOptions = (categories, level = 0) => {
        ...
        categories.forEach(cat => {
            if (category && category.id === cat.id) return;
            const prefix = '\u00A0\u00A0'.repeat(level) + (level > 0 ? '↳ ' : '');
            const catName = cat.translations.find(t => t.languageCode === 'vi')?.name || 'N/A';
            options.push(<option key={cat.id} value={cat.id}>{prefix + catName}</option>);
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label className="block text-sm font-medium text-gray-700">Tên danh mục (Tiếng Việt)
                ...
                <input type="text" name="viName" value={formData.viName} onChange={handleChange}
                    ...
                    <button type="button" onClick={handleTranslate} disabled={isTranslating}
                            className="flex items-center gap-x-2 flex-shrink-0 px-4 py-2 bg-gray-100 rounded-md hover:bg-gray-200 disabled:opacity-50 transition-colors">
                        <Languages size={18}/>
                        <span className="text-sm font-medium whitespace-nowrap">{isTranslating ? 'Đang dịch...' : 'Dịch'}</span>
                    </button>
                </div>
                {errors.viName && <p className="text-[12px] italic text-red-600 mt-2">{errors.viName}</p>}
            </div>
            <div>
                <label className="block text-sm font-medium text-gray-700">Tên danh mục (Tiếng Anh)</label>
                ...
                {errors.enName && <p className="text-[12px] italic text-red-600 mt-2">{errors.enName}</p>}
            </div>
            <div>
                ...
                <select name="parentId" value={formData.parentId || ''} onChange={handleChange}
                        className="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm bg-white focus:outline-none focus:ring-1 focus:ring-[var(--brand-blue)]">
            ...
            <div className="flex items-center space-x-3 pt-2">
                <input type="checkbox" id="showOnHomepage" name="showOnHomepage"
                       ...
                       className="h-4 w-4 text-brand-blue accent-brand-blue border-gray-300 rounded focus:ring-brand-blue disabled:opacity-50 disabled:cursor-not-allowed"
                />
                <label htmlFor="showOnHomepage"
                       className={`text-sm font-medium transition-colors ${formData.parentId ? 'text-gray-400' : 'text-gray-700'}`}>
